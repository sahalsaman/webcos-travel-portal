...
if (!MONGODB_URI || !SUPER_ADMIN_EMAIL || !SUPER_ADMIN_PASSWORD || SUPER_ADMIN_PASSWORD.length < 6) throw new Error('Set MONGODB_URI, SUPER_ADMIN_EMAIL and SUPER_ADMIN_PASSWORD (6+ characters).');
...
try {
 ...
 if (VOIBEE_DATABASE) {
  ...
  await db.collection('businesses').updateOne({ slug: 'voibee' }, { $set: { name: 'Voibee Holidays', databaseName: VOIBEE_DATABASE, hosts: [VOIBEE_PORTAL_HOST.toLowerCase()], siteUrl: VOIBEE_SITE_URL, logoUrl: `${VOIBEE_SITE_URL.replace(/\/$/, '')}/voibee-logo-with-name.png`, primaryColor: '#0060e6', status: 'active', provisioned: true }, $setOnInsert: { slug: 'voibee', createdAt: new Date() } }, { upsert: true });
 }
